...
// SetWhois - sets the whois a given name resolves to
func (k Keeper) SetWhois(ctx sdk.Context, name string, whois Whois) {
	if whois.Owner.Empty() {
		return
	}
	store := ctx.KVStore(k.storeKey)                            // Access store using storekey
	store.Set([]byte(name), k.cdc.MustMarshalBinaryBare(whois)) // insert key-value bytes[] pair
}

// GetWhois - looks up the Whois of a given name
func (k Keeper) GetWhois(ctx sdk.Context, name string) Whois {
	store := ctx.KVStore(k.storeKey) // Access store using storekey
	if !store.Has([]byte(name)) {
		return NewWhois()
	}
	bz := store.Get([]byte(name)) // Get key-value pair
	var whois Whois
